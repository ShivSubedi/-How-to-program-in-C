    ...
    while (!feof(fp))
      {
        ...
        fscanf(fp,"%lf",&category1[n].distance);
        temp[n] = category1[n]; 
 ...
 if (res == 'A')
 {
  ...
  case 2:
  	 printf("\nEnter the DISTANCE RANGE to search:");
        ...
        for (i = 0; i < n; i++)
   {
      if (key2> category1[i].distance)
      {
         printf ("%s in city %s is at distance %.2f has price $%d\n", category1[i].hotel, category1[i].city, category1[i].distance, category1[i].price);
         found = 1; 
